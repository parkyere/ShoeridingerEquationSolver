...
- **Phase-specific/transient:** `grad_v_buf_` (vec4, 2.15 GB, mean_force/energy only — recomputable via the closed-form bare-Coulomb `grad V = Z·r_vec/r³`, with the nucleus cell handled as in `regularized_coulomb_potential`); `relax_half_`/`relax_kin_` (2.15 GB, relaxation only — freeable); one on-demand collapse orbital (1.07 GB, one at a time).
...
**Two distinct axes — don't conflate.** 512³ at the current ±80 Bohr box (`Grid1D{-80,80,256}`, `main.cpp:135`) merely halves the spacing (`h`: 0.625 → 0.3125) — sharper Coulomb cusp (the nucleus-cell average `-Z·C/h` deepens, closing more of the s-state grid gap), **no new bound states** (box still holds only n≤6). A **bigger basis** (n>6, Rydberg) is **box-limited, not resolution-limited**: `<n|psi>` truncates any orbital support beyond the psi box, so growing `n_max` needs a larger `xmax` (a bigger box), which is independent of grid density. Both are separate follow-ons; this scope only removes the atlas and keeps one on-demand `synthesize_state` for collapse plus the build-time dipole table.
